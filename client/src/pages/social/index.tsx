    ...
    return (
        <div className="min-h-screen bg-[#121214] pb-24">
            <div className="max-w-5xl mx-auto px-3 sm:px-4 py-4 sm:py-8 space-y-4 sm:space-y-6">
                ...
                {/* Tabs Navigation */}
                <div className="flex p-1 bg-[#252527] rounded-xl border border-[#94fbdd]/10">
                    {tabs.map((tab) => (
                        <button
                            key={tab.id}
                ...
                <div className="bg-[#252527] rounded-2xl border border-[#94fbdd]/10 overflow-hidden">
                    <button
                    ...
                    >
                        <div className="flex items-center gap-3">
                            <div className="p-2 bg-[#94fbdd]/10 rounded-xl">
                                <CalendarIcon className="h-6 w-6 text-[#94fbdd]" />
                            </div>
                        ...
                        <div className={`transform transition-transform duration-300 ${showSessions ? 'rotate-180' : ''}`}>
                            <div className="p-1 rounded-full bg-[#121214] border border-[#94fbdd]/10">
                    ...
                    {showSessions && (
                        <div className="p-4 border-t border-[#94fbdd]/10 bg-[#121214]/50">
                            <SharedSessionsList />
                        </div>
                    )}
                </div>
                ...
                {activeTab === 'messages' && (
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 h-[calc(100vh-280px)] min-h-[500px]">
                        {/* Liste Conversations */}
                        <div className={`bg-[#252527] rounded-2xl border border-[#94fbdd]/10 overflow-hidden flex flex-col ${selectedConversation ? 'hidden md:flex' : 'flex'}`}>
                            <div className="p-4 border-b border-[#94fbdd]/10 bg-[#121214]/30">
                            ...
                            {loadingConvs ? (
                                ...
                                <div className="divide-y divide-[#94fbdd]/5 overflow-y-auto flex-1">
                                    {conversations.map((conv: any) => (
                                        <button
                                            key={conv.id}
                                            onClick={() => handleSelectConversation(conv.id)}
                                            className={`w-full p-4 text-left hover:bg-[#121214]/50 transition-colors ${selectedConversation === conv.id ? 'bg-[#94fbdd]/10 border-l-4 border-[#94fbdd]' : 'border-l-4 border-transparent'
                                                }`}
                                        >
                                            <div className="flex items-start justify-between gap-3">
                                                <div className="flex-1 min-w-0">
                                                    <h3 className={`font-semibold truncate ${selectedConversation === conv.id ? 'text-[#94fbdd]' : 'text-white'}`}>
                                                ...
                                                {conv.unreadCount > 0 && (
                                                    <span className="bg-[#94fbdd] text-[#121214] text-xs font-bold px-2 py-0.5 rounded-full min-w-[20px] text-center">
                                                        {conv.unreadCount}
                                                    </span>
                                                )}
                                            </div>
                        ...
                        {/* Fenêtre Chat */}
                        <div className={`md:col-span-2 bg-[#252527] rounded-2xl border border-[#94fbdd]/10 flex flex-col overflow-hidden ${!selectedConversation ? 'hidden md:flex' : 'flex'}`}>
                            {selectedConversation ? (
                                <>
                                    {/* Header Chat */}
                                    <div className="p-4 border-b border-[#94fbdd]/10 bg-[#121214]/30 flex items-center gap-3">
                                            ...
                                            return (
                                                <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                                                    <div className={`max-w-[85%] sm:max-w-[70%] rounded-2xl px-4 py-3 ${isMe
                                                        ? 'bg-[#94fbdd] text-[#121214] rounded-tr-none'
                                                        : 'bg-[#121214] border border-[#94fbdd]/10 text-gray-200 rounded-tl-none'
                                                        }`}>
                                                        {!isMe && (
                                                            <p className="text-xs font-bold text-[#94fbdd] mb-1">
                                                                {msg.sender?.name}
                                                            </p>
                                                        )}
                                                        ...
                                                        {isSessionInvitation && sessionId && !isMe && (
                                                            <button
                                                                ...
                                                                className="mt-3 w-full px-3 py-2 bg-[#252527] hover:bg-[#121214] text-[#94fbdd] border border-[#94fbdd]/30 rounded-xl text-sm font-bold transition-all flex items-center justify-center gap-2"
                                                            >
                                    ...
                                    {/* Input Area */}
                                    <div className="p-3 sm:p-4 border-t border-[#94fbdd]/10 bg-[#121214]/30">
                                        <div className="flex gap-2 sm:gap-3">
                                            <input
                                                ...
                                                className="flex-1 bg-[#121214] border border-[#94fbdd]/20 rounded-xl px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-[#94fbdd] focus:ring-1 focus:ring-[#94fbdd] transition-all"
                                            />
                                            <button
                                                onClick={handleSendMessage}
                                                disabled={!messageInput.trim()}
                                                className="p-3 bg-[#94fbdd] text-[#121214] rounded-xl hover:bg-[#94fbdd]/90 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg shadow-[#94fbdd]/20 active:scale-95"
                                            >
                                ...
                                <div className="flex-1 flex flex-col items-center justify-center text-gray-500 p-8 text-center">
                                    <div className="w-20 h-20 bg-[#252527] rounded-full flex items-center justify-center mb-4 border border-[#94fbdd]/10">
                                        <ChatBubbleLeftRightIcon className="h-10 w-10 text-[#94fbdd]" />
                                    </div>
                ...
                {activeTab === 'friends' && (
                    <div className="space-y-6">
                        {/* Recherche */}
                        <div className="bg-[#252527] p-4 rounded-2xl border border-[#94fbdd]/10">
                            <div className="relative">
                                <MagnifyingGlassIcon className="h-5 w-5 absolute left-3 top-3.5 text-gray-400" />
                                <input
                                    type="text"
                                    placeholder="Rechercher un utilisateur..."
                                    className="w-full bg-[#121214] pl-10 pr-4 py-3 rounded-xl border border-[#94fbdd]/20 text-white placeholder-gray-500 focus:outline-none focus:border-[#94fbdd] focus:ring-1 focus:ring-[#94fbdd] transition-all"
                                    value={searchQuery}
                            ...
                            {searchResults.length > 0 && (
                                <div className="mt-4 space-y-2">
                                    {searchResults.map((user: any) => (
                                        <div key={user.id} className="flex items-center justify-between p-3 bg-[#121214] rounded-xl border border-[#94fbdd]/5">
                                            <div className="flex items-center gap-3">
                                                <div className="w-10 h-10 bg-[#252527] rounded-full overflow-hidden border border-[#94fbdd]/20">
                                            ...
                                            {user.status === 'NONE' && (
                                                <button
                                                    ...
                                                    className="p-2 bg-[#94fbdd]/10 text-[#94fbdd] rounded-lg hover:bg-[#94fbdd]/20 transition-colors"
                                                >
                                                    <PlusIcon className="h-5 w-5" />
                                                </button>
                                            )}
                                            {user.status === 'SENT' && <span className="text-xs text-gray-400 bg-[#252527] px-2 py-1 rounded-lg">Envoyé</span>}
                                            {user.status === 'FRIEND' && <span className="text-xs text-[#94fbdd] bg-[#94fbdd]/10 px-2 py-1 rounded-lg">Ami</span>}
                                        </div>
                                    ))}
                                </div>
                            )}
                        ...
                        {friendRequests.length > 0 && (
                            <div className="bg-[#252527] p-4 rounded-2xl border border-[#94fbdd]/10">
                                <h3 className="font-bold text-white mb-4 flex items-center gap-2">
                                    <div className="w-2 h-2 rounded-full bg-[#94fbdd]"></div>
                                    Demandes reçues
                                </h3>
                                <div className="space-y-3">
                                    {friendRequests.map((req: any) => (
                                        <div key={req.id} className="flex items-center justify-between p-3 bg-[#121214] rounded-xl border border-[#94fbdd]/5">
                                            <div className="flex items-center gap-3">
                                                <div className="w-10 h-10 bg-[#252527] rounded-full overflow-hidden border border-[#94fbdd]/20">
                                            ...
                                            <div className="flex gap-2">
                                                <button onClick={() => acceptFriend.mutate(req.id)} className="p-2 bg-[#94fbdd]/10 text-[#94fbdd] rounded-lg hover:bg-[#94fbdd]/20 transition-colors">
                            ...
                            {friends.length === 0 ? (
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                    {friends.map((f: any) => (
                                        <div
                                            key={f.id}
                                            className="flex items-center justify-between p-3 bg-[#121214] rounded-xl border border-[#94fbdd]/5 hover:border-[#94fbdd]/30 transition-all cursor-pointer group"
                                        ...
                                        >
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="font-medium text-white group-hover:text-[#94fbdd] transition-colors">{f.friend.name}</span>
                                            </div>
                                            <div className="p-2 bg-[#252527] rounded-lg text-gray-400 group-hover:text-[#94fbdd] group-hover:bg-[#94fbdd]/10 transition-colors">
                        ...
                        <div className="bg-[#252527] p-4 rounded-2xl border border-[#94fbdd]/10">
                            {!isCreatingGroup ? (
                                <button
                                    ...
                                    className="w-full py-4 border border-dashed border-[#94fbdd]/30 rounded-xl text-[#94fbdd] font-semibold hover:bg-[#94fbdd]/5 transition-all flex items-center justify-center gap-2"
                                >
                                    <PlusIcon className="h-5 w-5" />
                                ...
                                <div className="flex flex-col sm:flex-row gap-3">
                                    <input
                                        ...
                                        className="flex-1 bg-[#121214] border border-[#94fbdd]/20 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#94fbdd]"
                                        value={newGroupName}
                                    ...
                                    <div className="flex gap-2">
                                        <button
                                            ...
                                            className="px-6 py-3 bg-[#94fbdd] text-[#121214] rounded-xl font-bold disabled:opacity-50 hover:bg-[#94fbdd]/90 transition-colors"
                                        >
                                            Créer
                                        </button>
                                        <button
                                    ...
                                    {groupRequests.map((req: any) => (
                                        <div key={req.id} className="flex items-center justify-between p-3 bg-[#121214] rounded-xl border border-[#94fbdd]/5">
                                            <span className="text-gray-300 text-sm">
                                                <strong className="text-white">{req.sender?.name}</strong> vous invite à rejoindre <strong className="text-[#94fbdd]">{req.group?.name}</strong>
                                            </span>
                                            <button
                                                onClick={() => acceptGroup.mutate(req.id)}
                                                className="px-3 py-1.5 bg-[#94fbdd]/10 text-[#94fbdd] rounded-lg text-sm font-bold hover:bg-[#94fbdd]/20 transition-colors"
                                            >
                                                Accepter
                                            </button>
                                        </div>
                            ...
                            {myGroups.length === 0 ? (
                                ...
                                <div className="grid grid-cols-1 gap-3">
                                    {myGroups.map((group: any) => (
                                        <div
                                            key={group.id}
                                            className="p-4 bg-[#121214] border border-[#94fbdd]/10 rounded-xl hover:border-[#94fbdd]/30 transition-all flex justify-between items-center group"
                                        >
                                            <div onClick={() => openGroupChat(group.id)} className="cursor-pointer flex-1">
                                                <h4 className="font-bold text-white group-hover:text-[#94fbdd] transition-colors">{group.name}</h4>
                                            ...
                                            <div className="flex gap-2">
                                                ...
                                                <button
                                                    ...
                                                    className="p-2 text-[#94fbdd] hover:bg-[#94fbdd]/10 rounded-lg transition-colors"
            ...
            {managingGroup && (
                <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-[#252527] rounded-2xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto border border-[#94fbdd]/20 shadow-2xl">
                        {/* Header */}
                        <div className="flex justify-between items-center mb-6 border-b border-[#94fbdd]/10 pb-4">
                        ...
                        <div className="mb-6">
                            ...
                            <div className="flex gap-2">
                                <input
                                    ...
                                    className="flex-1 bg-[#121214] border border-[#94fbdd]/20 rounded-xl px-3 py-2 text-white focus:outline-none focus:border-[#94fbdd]"
                                    id="groupNameInput"
                                />
                                <button
                                    onClick={() => {
                                    ...
                                    className="bg-[#94fbdd] text-[#121214] px-4 py-2 rounded-xl font-bold hover:bg-[#94fbdd]/90"
                                >
                                    Sauver
                                </button>
                            </div>
                        ...
                        <div className="mb-6">
                            ...
                            <div className="space-y-2 max-h-48 overflow-y-auto pr-2">
                                {groupMembers.map((member: any) => (
                                    <div key={member.id} className="flex justify-between items-center p-2 bg-[#121214] rounded-lg border border-[#94fbdd]/5">
                                        <div className="flex items-center gap-2">
                                            <div className="w-8 h-8 bg-[#252527] rounded-full overflow-hidden border border-[#94fbdd]/20">
                                            ...
                                            <span className="text-gray-200 text-sm">{member.name} {member.id === user?.id && <span className="text-[#94fbdd] text-xs">(Moi)</span>}</span>
                                        </div>
                                        {member.id !== user?.id ? (
                                            <button
                        ...
                        <div>
                            ...
                            <div className="space-y-2 max-h-48 overflow-y-auto pr-2">
                                {friends
                                    ...
                                    .map((f: any) => (
                                        <div key={f.id} className="flex justify-between items-center p-2 bg-[#121214] rounded-lg border border-[#94fbdd]/5">
                                            ...
                                            <button
                                                ...
                                                className="text-[#94fbdd] font-bold text-xs bg-[#94fbdd]/10 px-3 py-1.5 rounded-lg hover:bg-[#94fbdd]/20 transition-colors"
                                            >
                                                Inviter
                                            </button>
                                        </div>
