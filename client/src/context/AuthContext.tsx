...
// Type pour la réponse de getCurrentUser
...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  // Effet : succès → mise à jour de l'utilisateur
  useEffect(() => {
    if (data?.user) {
      setUser(data.user)
      setInitialised(true)
  ...
  // Effet : erreur → nettoyage de session
  useEffect(() => {
    if (error) {
      clearSession()
      setInitialised(true)
  ...
  // Effet : pas de token → initialisé immédiatement
  useEffect(() => {
    if (!accessToken) {
      setInitialised(true)
    }
