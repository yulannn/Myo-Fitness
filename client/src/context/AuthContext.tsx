...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  const [accessToken, setAccessToken] = useState<string | null>(() => typeof window === 'undefined' ? null : window.localStorage.getItem(ACCESS_TOKEN_KEY));
  ...
  const { data, isFetching, error } = useQuery<MeResponse, Error>({
    queryKey: ['auth', 'me'],
    queryFn: () => { if (!accessToken) throw new Error('Missing access token'); return getCurrentUser(accessToken); },
  ...
  useEffect(() => { if (data?.user) { setUser(data.user); setInitialised(true); } }, [data]);
  useEffect(() => { if (error) { clearSession(); setInitialised(true); } }, [error]);
  useEffect(() => { if (!accessToken) setInitialised(true); }, [accessToken]);

  const applyAuthResult = useCallback((payload: AuthSuccessResponse) => {
    setUser(payload.user);
    setAccessToken(payload.accessToken);
    if (typeof window !== 'undefined') window.localStorage.setItem(ACCESS_TOKEN_KEY, payload.accessToken);
