...
export class ChatService {
    ...
    async getMessages(conversationId: string, userId: number, skip = 0, take = 50) {
        // Vérifier l'accès
    ...
    async updateMessage(messageId: string, dto: UpdateMessageDto, userId: number) {
    ...
    private async findExistingPrivateConversation(user1Id: number, user2Id: number) {
        const conversations = await this.prisma.conversation.findMany({
            where: {
