...
export class ProgramService {
  ...
  async create(createProgramDto: CreateTrainingProgramDto, userId: number) {
    ...
    const program = await this.iaService.generateProgram(
      fitnessProfile,
      createProgramDto.template,
    );
  ...
  private async validateFitnessProfile(profileId: number, userId: number) {
    ...
    if (!profile)
      throw new ForbiddenException(
        'Profil fitness introuvable ou non autorisé',
      );
    return profile;
  }
  ...
  private async validateExercisesExist(sessions: any[]): Promise<number[]> {
    ...
    if (missing.length) {
      throw new BadRequestException(
        `Exercices manquants: ${missing.join(', ')}`,
      );
    }
  ...
  async createManualProgram(body: CreateManualProgramDto, userId: number) {
    ...
    await this.validateFitnessProfile(
      createProgramDto.fitnessProfileId,
      userId,
    );
    return this.prisma.$transaction(async (prisma) => {
  ...
  async addSessionToProgram(
    ...
    if (sessionNumber >= MAX_SESSIONS_PER_PROGRAM)
      throw new BadRequestException(
        `Le programme a déjà ${MAX_SESSIONS_PER_PROGRAM} sessions actives. Complétez-en une avant d'en ajouter.`,
      );
  ...
  async addCardioToProgram(
    ...
    if (exercise.type !== 'CARDIO') {
      throw new BadRequestException(
        "L'exercice sélectionné n'est pas de type CARDIO",
      );
    }

    return this.prisma.$transaction(async (prisma) => {
      // D'abord, supprimer les exercices cardio existants de tous les templates
      await this.removeCardioFromAllTemplates(
        prisma,
        program.sessionTemplates.map((t) => t.id),
      );
      ...
      return {
        success: true,
        message: 'Exercice cardio ajouté à tous les templates',
      };
    });
  }
  ...
  async removeCardioFromProgram(programId: number, userId: number) {
    ...
    return this.prisma.$transaction(async (prisma) => {
      ...
      return {
        success: true,
        message: 'Exercices cardio supprimés de tous les templates',
      };
    });
  }

  // Helper privé pour supprimer les exercices cardio de multiples templates
  private async removeCardioFromAllTemplates(
    prisma: any,
    templateIds: number[],
  ) {
