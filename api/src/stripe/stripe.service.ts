...
export class StripeService {
  ...
  async verifyCheckoutSession(sessionId: string) {
    ...
    if (
      ...
      try {
        ...
        console.error(
          `❌ Fallback activation failed for user ${userId}:`,
          error,
        );
      }
    }
  ...
  private async handleCheckoutSessionCompleted(
    ...
    const subscriptionPlan =
      plan === 'monthly' ? SubscriptionPlan.MONTHLY : SubscriptionPlan.YEARLY;
    ...
    if (actualPriceId !== expectedPriceId) {
      ...
      throw new BadRequestException(
        'Price validation failed: subscription does not match expected plan',
      );
    }
  ...
  private async handleSubscriptionUpdated(subscription: any) {
    ...
    if (!userId) {
      ...
      console.error(
        '   ⚠️  This subscription was likely created manually or is missing metadata',
      );
      return;
    }
