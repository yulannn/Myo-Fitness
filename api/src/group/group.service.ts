...
export class GroupService {
  ...
  async sendGroupRequest(
    ...
    const request = await this.prisma.friendGroupRequest.create({
      ...
      include: {
        // Include sender/group data for the notification payload
  ...
  async acceptGroupRequest(requestId: string) {
    ...
    if (conversation) {
      await this.prisma.conversationParticipant
        ...
        .catch((e) =>
          console.error('Error adding participant to conversation', e),
        );
  ...
  async leaveGroup(userId: number, groupId: number) {
    ...
    if (group.adminId === userId) {
      throw new ConflictException(
        'Admin cannot leave group. Delete group or transfer ownership.',
      );
    }
