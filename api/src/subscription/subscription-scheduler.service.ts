...
export class SubscriptionSchedulerService {
    ...
    async handleExpiredSubscriptions() {
        ...
        try {
            const count = await this.subscriptionService.checkAndUpdateExpiredSubscriptions();

            if (count > 0) {
                this.logger.log(`✅ Successfully marked ${count} subscription(s) as expired`);
            } else {
    ...
    async handleExpiredSubscriptionsFrequent() {
        ...
        try {
            ...
            if (count > 0) {
                this.logger.warn(`⚠️  Found ${count} expired subscription(s) in quick check`);
            }
        } catch (error) {
