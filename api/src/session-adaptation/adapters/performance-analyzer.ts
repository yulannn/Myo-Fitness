...
export class PerformanceAnalyzer {
    ...
    analyze(
        ...
        const successfulSets = performances.filter(p => p.success === true).length;
        const successRate = totalSets > 0 ? successfulSets / totalSets : 0;

        // 2. Performance sur les reps
        const avgRepsCompleted = totalSets > 0
            ? performances.reduce((sum, p) => sum + (p.reps_effectuees || 0), 0) / totalSets
            : 0;
        const repsCompletionRate = targetReps > 0 ? avgRepsCompleted / targetReps : 1;
        ...
        if (targetWeight && targetWeight > 0) {
            const performancesWithWeight = performances.filter(p => p.weight != null && p.weight > 0);
            if (performancesWithWeight.length > 0) {
                avgWeightUsed = performancesWithWeight.reduce((sum, p) => sum + p.weight, 0) / performancesWithWeight.length;
        ...
        const consistencyScore = this.calculateConsistency(performances, targetReps);
    ...
    private detectFailurePattern(performances: any[]): FailurePattern {
        const failures = performances.map((p, idx) => ({ index: idx, failed: !p.success }));
        const failureCount = failures.filter(f => f.failed).length;

        if (failureCount === 0) return FailurePattern.NONE;

        const totalSets = performances.length;
        const firstHalfFailures = failures.slice(0, Math.ceil(totalSets / 2)).filter(f => f.failed).length;
     ...
     */
    private calculateConsistency(performances: any[], targetReps: number): number {
