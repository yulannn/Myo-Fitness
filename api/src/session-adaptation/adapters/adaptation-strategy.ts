...
export class AdaptationStrategyEngine {
  ...
  private strategyCrushingIt(
    ...
    if (isBodyweight) {
      return {
        ...
        reasoning:
          'Performance excellente - Augmentation du volume (reps ou sets)',
        confidence: 0.9,
      };
    } else {
      ...
      return {
        type: canIncreaseWeight
          ? AdaptationType.INTENSITY
          : AdaptationType.VOLUME,
        priority: canIncreaseWeight
          ? AdaptationPriority.WEIGHT
          : AdaptationPriority.REPS,
        magnitude: AdaptationMagnitude.NORMAL,
  ...
  private strategyPerfect(): AdaptationStrategy {
    return {
      ...
      magnitude: AdaptationMagnitude.MICRO,
      reasoning:
        'Zone idéale de progression - Maintien pour consolider les acquis',
      confidence: 1.0,
    };
  }
  ...
  private strategyChallenging(isBodyweight: boolean): AdaptationStrategy {
    return {
      ...
      magnitude: AdaptationMagnitude.MICRO,
      reasoning:
        'Difficulté gérée - Maintien pour évaluer sur 1-2 séances supplémentaires',
      confidence: 0.7,
    };
  }
