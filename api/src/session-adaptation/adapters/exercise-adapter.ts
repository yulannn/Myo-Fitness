...
export class ExerciseAdapter {
  ...
  adapt(
    ...
    newReps = this.clamp(
      newReps,
      this.constraints.minReps,
      this.constraints.maxReps,
    );
    newSets = this.clamp(
      newSets,
      this.constraints.minSets,
      this.constraints.maxSets,
    );
    ...
    const weightChange =
      newWeight !== null && currentWeight !== null
    ...
    switch (strategy.type) {
      ...
      case AdaptationType.DENSITY:
        if (currentReps >= 15) {
          ...
          const increment =
            strategy.magnitude === AdaptationMagnitude.MICRO ? 1 : 2;
          return {
      ...
      case AdaptationType.DELOAD:
        const decrement =
          strategy.magnitude === AdaptationMagnitude.MACRO ? 3 : 2;
        return {
  ...
  private deload(
    ...
    const deloadFactor = magnitude === AdaptationMagnitude.MACRO ? 0.7 : 0.85;
